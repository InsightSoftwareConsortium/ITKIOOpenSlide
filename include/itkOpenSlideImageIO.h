...
public:
   ...
   * Call ReadImageInformation() again after calling this function. */
  virtual void
  SetAssociatedImageName(const std::string & strName);
   ...
   * Call ReadImageInformation() again after calling this function. */
  virtual bool
  SetLevelForDownsampleFactor(double dDownsampleFactor);

  /** Returns all associated image names stored in the file. */
  virtual AssociatedImageNameContainer
  ...
  virtual int64_t
  ComputeMaximumNumberOfStreamableRegions(const ImageIORegion & clRegion) const;

  /** Returns the minimum streamable region. */
  virtual ImageIORegion
  GetMinimumStreamableRegion() const;
